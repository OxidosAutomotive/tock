...
//! Test the implementation of HMAC-SHA1 by performing a hash and
...
use kernel::hil::digest::HmacSha1;
...
pub struct TestHmacSha1<'a, H: digest::Digest<'a, 32>> {
    hmac: &'a H,
...
impl<'a, H: digest::Digest<'a, 32> + HmacSha1> TestHmacSha1<'a, H> {
    ...
    pub fn run(&'static self) {
        ...
        let r = self.hmac.set_mode_hmacsha1(key);
        if r.is_err() {
        ...
        if r.is_err() {
            panic!("HmacSha1Test: failed to add data: {:?}", r);
        }
    }
}

impl<'a, H: digest::Digest<'a, 32>> digest::ClientData<32> for TestHmacSha1<'a, H> {
    ...
    fn add_mut_data_done(&self, result: Result<(), ErrorCode>, data: SubSliceMut<'static, u8>) {
        ...
        match result {
            Ok(()) => {}
            Err(e) => {
                kernel::debug!("HmacSha1Test: failed to add data: {:?}", e);
        ...
        match r {
            Ok(()) => {}
            Err((e, d)) => {
                kernel::debug!("HmacSha1Test: failed to run HMAC: {:?}", e);
...
impl<'a, H: digest::Digest<'a, 32>> digest::ClientHash<32> for TestHmacSha1<'a, H> {
    fn hash_done(&self, _result: Result<(), ErrorCode>, digest: &'static mut [u8; 32]) {
        ...
        if !error {
            kernel::debug!("HMAC-SHA1 matches!");
            self.client.map(|client| {
                client.done(Ok(()));
            });
        } else {
            kernel::debug!("HmacSha1Test: incorrect HMAC output!");
            self.client.map(|client| {
...
impl<'a, H: digest::Digest<'a, 32>> digest::ClientVerify<32> for TestHmacSha1<'a, H> {
    fn verification_done(&self, _result: Result<bool, ErrorCode>, _compare: &'static mut [u8; 32]) {
    }
}

impl<'a, H: digest::Digest<'a, 32>> CapsuleTest for TestHmacSha1<'a, H> {
