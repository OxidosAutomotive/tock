...
impl<'a, H: digest::Digest<'a, 32> + digest::Sha1> TestSha1<'a, H> {
    ...
    pub fn run(&'a self) {
        let r = self.sha.set_mode_sha1();
        if r.is_err() {
            panic!("Sha1Test: failed to set mode: {:?}", r)
        }
        ...
        if r.is_err() {
            panic!("Sha1Test: failed to add data: {:?}", r);
        }
    }
}
            ...
            if self.position.get() < data.len() {
                ...
                match result {
                    Ok(()) => {
                        ...
                        if v.is_err() {
                            panic!("Sha1Test: failed to verify: {:?}", v);
                        }
                    }
                    Err(e) => {
                        panic!("Sha1Test: adding data failed: {:?}", e);
                    }
                }
            }
        }
        ...
        match result {
            Ok(success) => {
                if success != self.correct.get() {
                    panic!(
                        "Sha1Test: Verification should have been {}, was {}",
                        self.correct.get(),
                        success
                    );
                } else {
            ...
            Err(e) => {
                panic!("Sha1Test: Error in verification: {:?}", e);
            }
        }
    }
}
