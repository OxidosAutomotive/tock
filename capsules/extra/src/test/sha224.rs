...
pub struct TestSha224<'a, H: digest::Digest<'a, 32>> {
    sha: &'a H,
...
impl<'a, H: digest::Digest<'a, 32> + digest::Sha224> TestSha224<'a, H> {
    pub fn new(
        sha: &'a H,
        data: &'static mut [u8],
    ...
    pub fn run(&'a self) {
        let r = self.sha.set_mode_sha224();
        if r.is_err() {
            panic!("Sha224Test: failed to set mode: {:?}", r)
        }
        ...
        if r.is_err() {
            panic!("Sha224Test: failed to add data: {:?}", r);
        }
    }
}

impl<'a, H: digest::Digest<'a, 32>> digest::ClientData<32> for TestSha224<'a, H> {
            ...
            if self.position.get() < data.len() {
                ...
                match result {
                    Ok(()) => {
                        ...
                        if v.is_err() {
                            panic!("Sha224Test: failed to verify: {:?}", v);
                        }
                    }
                    Err(e) => {
                        panic!("Sha224Test: adding data failed: {:?}", e);
                    }
                }
            }
        }
...
impl<'a, H: digest::Digest<'a, 32>> digest::ClientVerify<32> for TestSha224<'a, H> {
...
impl<'a, H: digest::Digest<'a, 32>> digest::ClientHash<32> for TestSha224<'a, H> {
    fn hash_done(&self, _result: Result<(), ErrorCode>, _digest: &'static mut [u8; 32]) {}
}

impl<'a, H: digest::Digest<'a, 32>> CapsuleTest for TestSha224<'a, H> {
