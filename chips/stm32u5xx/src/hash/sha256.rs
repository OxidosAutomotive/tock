...
pub struct Sha256Adapter<'a> {
    hash: &'a Hash<'a>,
    client: OptionalCell<HashClient<'a, SHA256_DIGEST_LEN>>,
}

impl<'a> Sha256Adapter<'a> {
...
impl<'a> HashAdapter<'a, Hash<'a>> for Sha256Adapter<'a> {
    fn add_data_done(
        &self,
...
impl<'a> digest::DigestData<'a, SHA256_DIGEST_LEN> for Sha256Adapter<'a> {
    fn set_data_client(
        &'a self,
        client: &'a dyn kernel::hil::digest::ClientData<SHA256_DIGEST_LEN>,
    ) {
...
impl<'a> digest::DigestHash<'a, SHA256_DIGEST_LEN> for Sha256Adapter<'a> {
    fn set_hash_client(
        &'a self,
        client: &'a dyn kernel::hil::digest::ClientHash<SHA256_DIGEST_LEN>,
    ) {
    ...
    ) -> Result<(), (kernel::ErrorCode, &'static mut [u8; SHA256_DIGEST_LEN])> {
        self.hash.run(digest).map_err(|(e, buf)| {
            //terrible
            let correct_buf: &'static mut [u8; SHA256_DIGEST_LEN] = buf.try_into().unwrap();
            (e, correct_buf)
        })
...
impl<'a> digest::DigestVerify<'a, SHA256_DIGEST_LEN> for Sha256Adapter<'a> {
    fn set_verify_client(
        &'a self,
        client: &'a dyn kernel::hil::digest::ClientVerify<SHA256_DIGEST_LEN>,
    ) {
...
impl<'a> digest::Digest<'a, SHA256_DIGEST_LEN> for Sha256Adapter<'a> {
    fn set_client(&'a self, client: &'a dyn digest::Client<SHA256_DIGEST_LEN>) {
        self.client.set(HashClient::Full(client));
    }
}

impl<'a> digest::DigestDataHash<'a, SHA256_DIGEST_LEN> for Sha256Adapter<'a> {
    fn set_client(&'a self, client: &'a dyn digest::ClientDataHash<SHA256_DIGEST_LEN>) {
        self.client.set(HashClient::DoubleHash(client));
    }
}

impl<'a> digest::DigestDataVerify<'a, SHA256_DIGEST_LEN> for Sha256Adapter<'a> {
    fn set_client(&'a self, client: &'a dyn digest::ClientDataVerify<SHA256_DIGEST_LEN>) {
        self.client.set(HashClient::DoubleVerify(client));
    }
}

impl digest::Sha256 for Sha256Adapter<'_> {
    fn set_mode_sha256(&self) -> Result<(), ErrorCode> {
        self.hash.set_mode_sha256()
    }
}

impl digest::HmacSha256 for Sha256Adapter<'_> {
    fn set_mode_hmacsha256(&self, key: &[u8]) -> Result<(), ErrorCode> {
        self.hash.set_mode_hmacsha256(key)
    }
}

impl digest::Md5 for Sha256Adapter<'_> {
    fn set_mode_md5(&self) -> Result<(), ErrorCode> {
        Err(ErrorCode::NOSUPPORT)
    }
}
impl digest::HmacMd5 for Sha256Adapter<'_> {
    fn set_mode_hmacmd5(&self, _key: &[u8]) -> Result<(), ErrorCode> {
        Err(ErrorCode::NOSUPPORT)
    }
}

impl digest::Sha1 for Sha256Adapter<'_> {
    fn set_mode_sha1(&self) -> Result<(), ErrorCode> {
        Err(ErrorCode::NOSUPPORT)
    }
}
impl digest::HmacSha1 for Sha256Adapter<'_> {
    fn set_mode_hmacsha1(&self, _key: &[u8]) -> Result<(), ErrorCode> {
        Err(ErrorCode::NOSUPPORT)
    }
}

impl digest::Sha224 for Sha256Adapter<'_> {
    fn set_mode_sha224(&self) -> Result<(), ErrorCode> {
        Err(ErrorCode::NOSUPPORT)
    }
}
impl digest::HmacSha224 for Sha256Adapter<'_> {
    fn set_mode_hmacsha224(&self, _key: &[u8]) -> Result<(), ErrorCode> {
        Err(ErrorCode::NOSUPPORT)
    }
}

impl digest::Sha384 for Sha256Adapter<'_> {
    fn set_mode_sha384(&self) -> Result<(), ErrorCode> {
        Err(ErrorCode::NOSUPPORT)
    }
}
impl digest::HmacSha384 for Sha256Adapter<'_> {
    fn set_mode_hmacsha384(&self, _key: &[u8]) -> Result<(), ErrorCode> {
        Err(ErrorCode::NOSUPPORT)
    }
}

impl digest::Sha512 for Sha256Adapter<'_> {
    fn set_mode_sha512(&self) -> Result<(), ErrorCode> {
        Err(ErrorCode::NOSUPPORT)
    }
}
impl digest::HmacSha512 for Sha256Adapter<'_> {
